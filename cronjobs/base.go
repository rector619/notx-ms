...
type CronJob func(extReq request.ExternalRequest, db *mongodb.Database)
...
func UpdateCronJobInterval(extReq request.ExternalRequest, db *mongodb.Database, jobName string, number int, base string) error {
	var (
		interval time.Duration
	)
...
func Scheduler(extReq request.ExternalRequest, db *mongodb.Database, mutex *sync.Mutex, jobName string, cronJob CronJob, interval time.Duration) {
	for {
		select {
		default:
			mutex.Lock()
...
func StartCronJob(extReq request.ExternalRequest, db *mongodb.Database, jobName string) {
	mutex := &sync.Mutex{}
	jobName = strings.ToLower(jobName)
	cronJob, ok := cronJobs[jobName]
	if ok {
...
func RestartCronJob(extReq request.ExternalRequest, db *mongodb.Database, jobName string) {
	StopCronJob(jobName)
	StartCronJob(extReq, db, jobName)
}

func SetupCronJobs(extReq request.ExternalRequest, db *mongodb.Database, selectedJobs []string) {
	// mutex := &sync.Mutex{}
